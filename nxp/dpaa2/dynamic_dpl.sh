...
then
	...
	do
		...
		DPNI=$(restool -s dpni create --options=$DPNI_OPTIONS  $ADD_DPNI_OPTIONS --num-tcs=$MAX_TCS --num-queues=$MAX_QUEUES --fs-entries=$FS_ENTRIES --vlan-entries=16 --qos-entries=$MAX_QOS --num-cgs=$MAX_CGS --container=$DPRC)
		restool dprc sync
		if [[ $TYPE != "dpmac" ]]
		then
