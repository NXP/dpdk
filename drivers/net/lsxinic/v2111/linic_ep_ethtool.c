...
static int lsinic_if_init(struct rte_eth_dev *dev)
{
	...
	if (lsinic_v2111_reset_config_fromrc(adapter))
...
static int lsinic_if_link_up(struct rte_eth_dev *dev)
{
	...
	lsinic_dev_v2111_rx_enable_start(dev);
	lsinic_dev_v2111_tx_enable_start(dev);
...
static int lsinic_if_link_down(struct rte_eth_dev *dev)
{
	...
	lsinic_dev_v2111_rx_stop(dev, 1);
	lsinic_dev_v2111_tx_stop(dev, 1);
...
void *lsinic_v2111_poll_dev_cmd(void *arg __rte_unused)
{
	struct rte_lsx_pciep_device *dev;
