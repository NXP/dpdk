...
#include "linic_rc_rxtx.h"
#include "linic_rc_ethdev.h"
#include "linic_rc_hw.h"
...
static void *lxsnic_rc_debug_status(void *arg)
{
	...
	while (1) {
		...
		print_port_v2111_status(eth_dev, NULL, DEBUG_STATUS_INTERVAL,
			LSINIC_RC_PORT);
		printf("\r\n\r\n");
	}
...
{
	struct lxsnic_ring *rx_ring = rxq;
...
{
	struct lxsnic_ring *tx_ring = txq;
...
lxsnic_dev_close(struct rte_eth_dev *dev)
{
	...
	if (ret) {
		LSXINIC_PMD_ERR("%s removed failed(%d)",
			__func__, ret);
	}
}
...
eth_lsnic_dev_init(struct rte_eth_dev *eth_dev)
{
	...
	eth_dev->rx_pkt_burst = lxsnic_v2111_eth_recv_pkts;
	/* < Pointer to PMD receive function. */
	eth_dev->tx_pkt_burst = lxsnic_v2111_eth_xmit_pkts;
	...
	lxsnic_v2111_pf_host_init(eth_dev);
...
eth_lxsnic_close(struct rte_eth_dev *dev __rte_unused)
{
	...
	if (adapter->num_vfs)
		lxsnic_v2111_disable_sriov(adapter);
...
RTE_PMD_EXPORT_NAME(net_lxsnic_v2111, __COUNTER__);
