...
uint16_t
lxsnic_v2111_eth_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts,
	uint16_t nb_pkts)
{
...
static void lxsnic_txq_loop(void)
{
	...
	/* Check if txq already added to list */
	TAILQ_FOREACH_SAFE(q, &RTE_PER_LCORE(lxsnic_txq_list), next, tq) {
		if (unlikely(q->multi_core_ring &&
			...
			if (ret) {
				xmit_ret = lxsnic_v2111_eth_xmit_pkts(q,
					tx_pkts, ret);
...
uint16_t
lxsnic_v2111_eth_recv_pkts(void *queue, struct rte_mbuf **rx_pkts,
	uint16_t nb_pkts)
{
