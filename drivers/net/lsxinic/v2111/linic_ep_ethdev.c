...
static struct eth_dev_ops lsinic_eth_dev_ops = {
	...
	.rx_queue_setup       = lsinic_dev_v2111_rx_queue_setup,
	.rx_queue_release     = lsinic_dev_v2111_rx_queue_release,
	.tx_queue_setup       = lsinic_dev_v2111_tx_queue_setup,
	.tx_queue_release     = lsinic_dev_v2111_tx_queue_release,
...
#ifdef RTE_LSINIC_PKT_MERGE_ACROSS_PCIE
int
lsinic_v2111_split_dev_flow_create(struct lsinic_adapter *adapter)
{
	...
	struct rte_lsx_pciep_device *lsinic_dev)
{
	...
	eth_dev->rx_pkt_burst = lsinic_v2111_recv_pkts;
	eth_dev->tx_pkt_burst = lsinic_v2111_xmit_pkts;
...
lsinic_dev_start(struct rte_eth_dev *eth_dev)
{
	...
	lsinic_dev_v2111_tx_init(eth_dev);

	/* This can fail when allocating mbufs for descriptor rings */
	err = lsinic_dev_v2111_rx_init(eth_dev);
	if (err) {
		LSXINIC_PMD_ERR("Unable to initialize RX hardware");
		lsinic_dev_v2111_clear_queues(eth_dev);
		return -EIO;
	}
	...
	if (!thread_init_flag) {
		if (pthread_create(&thread, NULL,
			lsinic_v2111_poll_dev_cmd, NULL)) {
...
lsinic_dev_stop(struct rte_eth_dev *dev)
{
	...
	rx_stop = lsinic_dev_v2111_rx_stop(dev, 0);
	tx_stop = lsinic_dev_v2111_tx_stop(dev, 0);
	...
	lsinic_dev_v2111_clear_queues(dev);
}
...
int
lsinic_v2111_reset_config_fromrc(struct lsinic_adapter *adapter)
{
