...
int lsxvio_logtype_pmd;
#define RTE_LOGTYPE_LSXVIO_NET lsxvio_logtype_pmd

#define LSXVIO_PMD_LOG(level, fmt, args...) \
	RTE_LOG_LINE(level, LSXVIO_NET, "lsxvio:" fmt, ##args)
	...
	uint16_t vtpci_queue_idx)
{
	...
	if (vq_size == 0) {
		LSXVIO_PMD_ERR("virtqueue does not exist");
		return -EINVAL;
	}

	if (!virtio_with_packed_queue(hw) && !rte_is_power_of_2(vq_size)) {
		LSXVIO_PMD_ERR("split virtqueue size is not power of 2");
		return -EINVAL;
	}

	snprintf(vq_name, sizeof(vq_name), "port%d_vq%d", dev->data->port_id, vtpci_queue_idx);

	vq = virtqueue_alloc(hw, vtpci_queue_idx, vq_size, queue_type, numa_node, vq_name);
	if (!vq) {
		LSXVIO_PMD_ERR("virtqueue init failed");
		return -ENOMEM;
	}
	...
	if (VIRTIO_OPS(hw)->setup_queue(hw, vq) < 0) {
		LSXVIO_PMD_ERR("setup_queue failed");
		ret = -EINVAL;
		goto clean_vq;
	}
...
lsxvio_rc_eth_stats_get(struct rte_eth_dev *dev,
	struct rte_eth_stats *stats, struct eth_queue_stats *qstats)
{
	lsxvio_rc_update_stats(dev, stats, qstats);
	...
	uint16_t queue_idx)
{
	...
	for (desc_idx = 0; desc_idx < RTE_PMD_VIRTIO_RX_MAX_BURST;
	     desc_idx++) {
		vq->rxq.sw_ring[vq->vq_nentries + desc_idx] =
			rxvq->fake_mbuf;
	}
	...
	struct rte_eth_dev_info *dev_info)
{
	...
	dev_info->speed_capa = RTE_ETH_LINK_SPEED_10G; /* fake value */
...
RTE_PMD_EXPORT_NAME(lsxvio_net);
RTE_PMD_REGISTER_PCI_TABLE(lsxvio_net, lsxvio_pci_id_map);
RTE_LOG_REGISTER_DEFAULT(lsxvio_logtype_pmd, NOTICE);
