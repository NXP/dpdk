...
#define LSX_PCIEP_EP_MEM_POOL_BAR_IDX (4)
...
enum lsinic_command {
	LSINIC_COMMAND_VF_MAC_ADDR = 0x01,
	LSINIC_COMMAND_VF_VLAN = 0x10,
	LSINIC_COMMAND_PF_MAC_ADDR = 0x1000
};
...
#define LSINIC_BD_128_TYPE 0
enum EP_MEM_BD_TYPE {
	EP_MEM_BD_128 = LSINIC_BD_128_TYPE,
...
enum RC_MEM_BD_TYPE {
	RC_MEM_BD_128 = LSINIC_BD_128_TYPE,
...
struct __rte_packed_begin lsinic_bdr_reg {
...
struct __rte_packed_begin lsinic_bd_desc_128 {
...
#define LSINIC_BD_DESC_64_ADDR_BW 48
#define LSINIC_BD_DESC_64_ADDR_MASK \
	((((uint64_t)1) << LSINIC_BD_DESC_64_ADDR_BW) - 1)

union __rte_packed_begin lsinic_bd_desc_64 {
	uint64_t desc;
	struct {
		uint64_t pkt_addr:LSINIC_BD_DESC_64_ADDR_BW;
		uint16_t len_cmd:16;
	};
} __rte_packed_end;
...
struct __rte_packed_begin lsinic_seg_desc_entry {
	uint32_t offset;
	uint32_t positive:1;
	uint32_t len:31;
} __rte_packed_end;

struct __rte_packed_begin lsinic_seg_desc {
	uint64_t base_addr;
...
struct __rte_packed_begin lsinic_rc_rx_len {
	uint16_t total_len;
} __rte_packed_end;

#define LSINIC_EP_TX_SEG_MAX_ENTRY (LSINIC_SG_DESC_MAX_ENTRY - 2)

struct __rte_packed_begin lsinic_rc_rx_seg {
	uint16_t len[LSINIC_EP_TX_SEG_MAX_ENTRY];
	uint8_t rsv[7];
	uint8_t nb;
} __rte_packed_end;

struct __rte_packed_begin lsinic_rc_tx_bd_cnf {
	uint8_t bd_complete;
} __rte_packed_end;

struct __rte_packed_begin lsinic_rc_tx_idx_cnf {
	uint32_t idx_complete;
} __rte_packed_end;

struct __rte_packed_begin lsinic_ep_tx_dst_addr {
	uint64_t pkt_addr;
} __rte_packed_end;

#define LSINIC_SEG_OFFSET_MAX (RTE_BIT64(31) - 1)
struct __rte_packed_begin lsinic_ep_tx_seg_entry {
	union {
		uint32_t seg_entry;
		struct {
			uint32_t positive:1;
...
struct __rte_packed_begin lsinic_ep_tx_seg_dst_addr {
	uint64_t addr_base;
...
#define LSXINIC_MAX(a, b, c) RTE_MAX(RTE_MAX(a, b), c)
...
#define LSINIC_LEN_RING_SIZE \
	(sizeof(struct lsinic_rc_rx_len) * LSINIC_BD_ENTRY_COUNT)
...
struct __rte_packed_begin lsinic_command_reg {
	uint32_t cmd_type;
	uint32_t cmd_vf_idx;
...
struct __rte_packed_begin lsinic_dev_reg {  /* offset 0x000-0x1FF */
...
struct __rte_packed_begin lsinic_rcs_reg {  /* offset 0x200-0x2FF */
...
#define LSXINIC_VF_AVAILABLE RTE_BIT32(15)

struct __rte_packed_begin lsinic_eth_reg {  /* offset 0x300-0x3FF */
	uint32_t rev;
	uint32_t eth_stat;
	uint32_t fmidx;
	uint32_t macidx;
