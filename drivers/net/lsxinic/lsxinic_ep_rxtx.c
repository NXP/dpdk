...
static inline void lsinic_sw_bd_reset(struct lsinic_queue *q)
{
	q->sw_bd_pool_cnt = 0;
}

static inline void lsinic_sw_bd_push(struct lsinic_queue *q,
...
lsinic_qdma_tx_multiple_enqueue(struct lsinic_queue *queue, bool append)
{
	...
	if ((queue->adapter->ep_cap & LSINIC_EP_CAP_TXQ_DMA_NO_RSP) &&
		nb_jobs) {
		bd_jobs_len = sizeof(struct lsinic_bd_desc) * nb_jobs;
		jobs_idx = jobs_avail_idx & (queue->nb_desc - 1);
	...
	if (rxq_dma_bd_nb > 0) {
		...
		bd_jobs_len = sizeof(struct lsinic_bd_desc) * rxq_dma_bd_nb;
		if ((rxq_dma_bd_start + rxq_dma_bd_nb) <= (int)rxq->nb_desc) {
			jobs[nb_jobs]->len = bd_jobs_len;
			rxq_bd_jobs_num = 1;
		} else {
			jobs[nb_jobs]->len = sizeof(struct lsinic_bd_desc) *
...
lsinic_tx_merge_cb(struct rte_mbuf *buf,
	struct dpaa2_queue *dpni_rxq)
{
	struct lsinic_queue *txq =
		(struct lsinic_queue *)dpni_rxq->lpbk_cntx;
	...
	lsinic_tx_merge_one_to_txq(txq, buf);
}
	...
	struct rte_mbuf **bufs, uint16_t nb_pkts)
{
	...
	do {
		...
		if (likely(lsinic_q)) {
			...
			if (lsinic_q->type == LSINIC_QUEUE_TX)
				lsinic_tx_merge_cb(bufs[num_rx], dpaa2_q);
			else
				lsinic_rx_split_cb(bufs[num_rx], dpaa2_q);
		} else {
			bufs[num_rx] = lsinic_dpaa2_fd_to_mbuf(fd,
							eth_data->port_id,
							LSINIC_QUEUE_TX);
			rte_pktmbuf_free(bufs[num_rx]);
		}
	...
	if (unlikely(!num_rx && lsinic_q &&
		lsinic_q->type == LSINIC_QUEUE_TX))
...
lsinic_hw_offload_queue_setup(struct lsinic_queue *q)
{
	...
	if (q->type == LSINIC_QUEUE_TX &&
		...
		if (adapter->ep_cap &
			LSINIC_EP_CAP_APP_HANDLE_RECYCLE_DEV) {
	...
	if (dpaa2_dev) {
		ret = dpaa2_dev_recycle_qp_setup(dpaa2_dev,
				q->queue_id, rx_lpbk, (uint64_t)q,
				&dpaa2_txq, &dpaa2_rxq);
		if (!ret) {
			q->dpni_txq = dpaa2_txq;
			q->dpni_rxq = dpaa2_rxq;
			if (q->type == LSINIC_QUEUE_TX &&
				dpaa2_txq) {
...
lsinic_queue_start(struct lsinic_queue *q)
{
	...
	uint64_t rc_bus_addr, ob_offset;
	int ret;
	...
	rc_bus_addr = LSINIC_READ_REG_64B((uint64_t *)(&q->ep_reg->r_descl));
	if (rc_bus_addr) {
	...
	for (i = 0; i < q->nb_desc; i++) {
		...
		e2r_bd_dma_jobs[i].src = DPAA2_VADDR_TO_IOVA(&q->ep_bd_desc[i]);
		e2r_bd_dma_jobs[i].dest = q->ob_base + rc_bus_addr +
			i * sizeof(struct lsinic_bd_desc);
	...
	for (i = 0; i < q->nb_desc; i++) {
		...
		r2e_bd_dma_jobs[i].dest =
			DPAA2_VADDR_TO_IOVA(&q->ep_bd_desc[i]);
		r2e_bd_dma_jobs[i].src =
			q->ob_base + rc_bus_addr +
			i * sizeof(struct lsinic_bd_desc);
...
static __rte_always_inline void
lsinic_bd_update_used_to_rc(struct lsinic_queue *queue, uint16_t used_idx)
{
#ifdef LSINIC_BD_CTX_IDX_USED
...
lsinic_tx_dma_dequeue(struct lsinic_queue *txq)
{
	...
	if (txq->pkts_eq == txq->pkts_dq && txq->dpni_txq)
		return 0;
...
static uint16_t
lsinic_dpni_merge(struct rte_mbuf **tx_pkts,
	uint16_t nb_pkts,
		...
		uint16_t nb_pkts, uint16_t bd_idx)
{
	...
	rte_mempool_put_bulk(tx_pkts[0]->pool, (void *)&tx_pkts[1], mg_num - 1);
	txq->packets += mg_num;
	ep_txd->len_cmd = LSINIC_BD_CMD_EOP | LSINIC_BD_CMD_MG;
	ep_txd->len_cmd |= (mg_len - sizeof(struct lsinic_mg_header));
...
static uint16_t
lsinic_dpni_enqueue(struct lsinic_queue *lsinic_q,
	struct qbman_fd *fd_arr, int fd_nb, bool sync)
{
	int ret;
	uint16_t loop = 0;
	struct qbman_eq_desc eqdesc;
	struct qbman_swp *swp;
	struct dpaa2_queue *txq = lsinic_q->dpni_txq;
	...
	lsinic_q->recycle_pending += fd_nb;

	if (unlikely(sync)) {
		struct rte_eth_dev_data *eth_data = txq->eth_data;
		int rx_count = 0;
		struct rte_mbuf **mbuf =
				rte_malloc(NULL, sizeof(void *) * fd_nb,
							64);

		if (!mbuf) {
			LSXINIC_PMD_ERR("mbuf alloc failed!");
			return loop;
		}

		while (1) {
			rte_eth_rx_burst(eth_data->port_id, 0, mbuf, 32);
			if (!lsinic_q->recycle_pending)
				break;
			rx_count++;
			if (rx_count > 100000)
				break;
		...
		if (lsinic_q->recycle_pending)
			LSXINIC_PMD_ERR("dpni enqueue sync failed!");
...
static uint16_t
lsinic_pre_xmit_pkts_dpni_merge(struct lsinic_queue *txq,
	struct rte_mbuf **tx_pkts,
	uint16_t nb_pkts)
{
	...
	direct_nb = lsinic_xmit_directly(tx_pkts, nb_pkts, txq, mg_pkts);

	nb_pkts -= direct_nb;

	while (nb_pkts) {
		if (((txq->mg_dsc_tail + 1) & (txq->nb_desc - 1)) ==
			txq->mg_dsc_head)
			break;
		mg_dsc = &txq->mg_dsc[txq->mg_dsc_tail];
		ret = lsinic_dpni_merge(&mg_pkts[tx_num],
				nb_pkts, &fd_gather[fd_num], mg_dsc,
				txq->dpni_merge_max);
		if (ret > 0) {
			txq->mg_dsc_tail = (txq->mg_dsc_tail + 1) & (txq->nb_desc - 1);
			fd_num++;
		}
		tx_num += ret;
		nb_pkts -= ret;
	...
	if (fd_num > 0)
		lsinic_dpni_enqueue(txq, fd_gather, fd_num, false);
	...
	uint16_t nb_pkts)
{
	...
	if (txq->dpni_txq) {
		return lsinic_pre_xmit_pkts_dpni_merge(txq,
				tx_pkts, nb_pkts);
	}
...
lsinic_pci_dma_test_init(struct lsinic_queue *queue)
{
	...
	uint16_t pkt_len = queue->adapter->merge_threshold;
	...
	if (pkt_len < (RTE_ETHER_MIN_LEN - RTE_ETHER_CRC_LEN) ||
		pkt_len > 4096)
		pkt_len = queue->adapter->merge_threshold;

	dma_test->mbufs =
		rte_zmalloc(NULL, sizeof(struct rte_mbuf *) *
					queue->nb_desc, 64);
	if (!dma_test->mbufs)
		return -ENOMEM;
	...
	jobs = rte_zmalloc(NULL, sizeof(struct rte_qdma_job) *
						queue->nb_desc, 64);
	...
	LSXINIC_PMD_INFO("qDMA-PCIe benchmark core%d pkt(%dB) %s",
		rte_lcore_id(), pkt_len,
		queue->type == LSINIC_QUEUE_RX ?
...
lsinic_pci_dma_rx_tx_test(struct lsinic_queue *rx_queue)
{
	...
	dma_rxq = rx_queue->dma_vq;
	dma_txq = tx_queue->dma_vq;
...
lsinic_recv_bd(struct lsinic_queue *rx_queue)
{
	...
	do {
		...
		rte_lsinic_prefetch(rxdp + 1);
		if ((rxdp->bd_status & RING_BD_STATUS_MASK) !=
				RING_BD_AVAILABLE) {
			rxdp = LSINIC_RC_BD_DESC(rxq, bd_idx);
		...
		LSXINIC_PMD_DBG("port%d rxq%d next_avail_idx=%d "
			"bd_idx=%d",
			rxq->port_id, rxq->queue_id,
			rxq->next_avail_idx, bd_idx);

		rte_memcpy(&rxe->bd, rxdp, sizeof(struct lsinic_bd_desc));
		...
		if (1) {
			...
			if (!rte_is_aligned((void *)dma_job->src, 64)) {
				LSXINIC_PMD_DBG("RXQ[%" PRIu32 "]"
					" dma_job->src is not aligned"
					" again 64 0x%" PRIx64 "\n",
					rxq->queue_id,
					dma_job->src);
			}
			...
			LSXINIC_PMD_DBG("port%d rxq%d BD%d src:0x%lx dst:0x%lx",
				rxq->port_id,
				rxq->queue_id,
				bd_idx,
				(unsigned long)dma_job->src,
				(unsigned long)dma_job->dest);
			...
			/*rxq->nb_in_dma++;*/
			total_bytes += size + rxe->align_dma_offset;
			total_packets++;
		}
	...
	if (rxq && rxq->pair) {
		...
		if (adapter->ep_cap & LSINIC_EP_CAP_HW_MERGE_PKTS &&
			...
			struct rte_eth_dev *offload_dev =
				adapter->merge_dev->eth_dev;

			if (txq->dpni_rxq) {
				struct rte_mbuf *mbuf_merged[DEFAULT_TX_RS_THRESH];

				ret = offload_dev->rx_pkt_burst(txq->dpni_rxq,
						mbuf_merged,
...
lsinic_rxq_loop(struct lsinic_queue *rxq)
{
	...
	if ((rxq->adapter->ep_cap &
		...
		if (likely(offload_dev->data->dev_started)) {
			ret = offload_dev->rx_pkt_burst(rxq->dpni_rxq,
					mbuf_split,
					DEFAULT_TX_RS_THRESH);
			if (likely(ret > 0)) {
				for (i = 0; i < ret; i++)
					lsinic_rx_split_cb(mbuf_split[i], rxq->dpni_rxq);
			}
		}
	}
}
...
static uint16_t lsinic_dpni_split_mbuf_to_fd(
...
lsinic_recv_pkts_to_cache(struct lsinic_queue *rxq)
{
	...
	uint16_t bd_idx;
	uint16_t nb_rx = 0;
	uint16_t rx_count, len;
	struct lsinic_bd_desc *rxdp = NULL;
	int count = 0, fd_idx = 0, ret, fd_start, fd_count, idx;
	struct rte_qdma_job *dma_job;
	struct qbman_fd *fd_arr = rxq->fd_arr;
	...
	while (nb_rx < rx_count) {
		...
		/* Get rte_mbuf of received packet */
		if (adapter->cap & LSINIC_CAP_XFER_COMPLETE) {
		...
		if (rxdp->len_cmd & LSINIC_BD_CMD_MG) {
			...
			if (rxq->split_type == LSINIC_HW_SPLIT) {
				if ((fd_idx + count) >
					(LSINIC_MERGE_MAX_NUM * 2)) {
					fd_start = 0;
					fd_count = fd_idx;
					while (fd_count) {
						ret = lsinic_dpni_enqueue(rxq,
								&fd_arr[fd_start], fd_count,
								false);
						fd_start += ret;
						fd_count -= ret;
					}
				...
				count = lsinic_dpni_split_mbuf_to_fd(rxm,
							&fd_arr[fd_idx], count);
				for (idx = fd_idx;
					idx < (fd_idx + count);
					idx++) {
					len = DPAA2_GET_FD_LEN(&fd_arr[idx]);
					rxq->bytes += len;
					rxq->bytes_fcs += len +
						LSINIC_ETH_FCS_SIZE;
				...
				rxq->new_desc += count;
				fd_idx += count;
			} else if (rxq->split_type == LSINIC_MBUF_CLONE_SPLIT) {
			...
			if (rxq->split_type == LSINIC_HW_SPLIT) {
				/* Single buffer still needs to be enqueue to
				 * DPNI for order preserve.
				 */
				RTE_ASSERT(rxq->dpni_txq);
				if ((fd_idx + 1) > LSINIC_MERGE_MAX_NUM) {
					fd_start = 0;
					fd_count = fd_idx;
					while (fd_count) {
						ret = lsinic_dpni_enqueue(rxq,
				...
				count = lsinic_dpni_split_mbuf_to_fd(rxm,
							&fd_arr[fd_idx], 0);
				rxq->packets++;
				len = DPAA2_GET_FD_LEN(&fd_arr[fd_idx]);
				rxq->bytes += len;
				rxq->bytes_fcs += len +
					LSINIC_ETH_FCS_SIZE;
				rxq->bytes_overhead += len +
