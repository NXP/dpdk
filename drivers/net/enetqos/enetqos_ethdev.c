...
reset_poll_timeout(void *ioaddr, uint32_t value, int delay_us, int timeout_us)
{
	...
	do {
		value = rte_read32((void *)((size_t)ioaddr + EQOS_DMA_MODE));
		rte_delay_us(delay_us);
		wait_time += delay_us;

		if (!(value & EQOS_DMA_BUS_MODE_SFT_RESET))
			return ret;
...
{
	uint32_t value = rte_read32((void *)((size_t)ioaddr + EQOS_DMA_MODE));

	/* DMA SW reset */
	value |= EQOS_DMA_BUS_MODE_SFT_RESET;
	rte_write32(value, (void *)((size_t) ioaddr + EQOS_DMA_MODE));

	return reset_poll_timeout((void *)((size_t)ioaddr + EQOS_DMA_MODE),
			value, 10000, 1000000);
}
	...
	struct enetqos_dma_cfg *dma_cfg, uint32_t chan)
{
	...
	value = rte_read32((void *)((size_t)ioaddr + EQOS_DMA_CHAN_CONTROL(chan)));

	if (dma_cfg->pblx8)
		value = value | EQOS_DMA_BUS_MODE_PBL;

	rte_write32(value, (void *)((size_t)ioaddr + EQOS_DMA_CHAN_CONTROL(chan)));
}
...
{
	uint32_t value =
		rte_read32((void *)((size_t) ioaddr + EQOS_DMA_SYS_BUS_MODE));

	/* Set the Fixed burst mode*/
	if (dma_cfg->fixed_burst)
		value |= EQOS_DMA_SYS_BUS_FB;

	/* Mixed Burst has no effect when fb mode is set*/
	if (dma_cfg->mixed_burst)
		value |= EQOS_DMA_SYS_BUS_MB;

	if (dma_cfg->aal)
		value |= EQOS_DMA_SYS_BUS_AAL;

	if (dma_cfg->eame)
		value |= EQOS_DMA_SYS_BUS_EAME;

	rte_write32(value, (void *)((size_t) ioaddr + EQOS_DMA_SYS_BUS_MODE));

	value = rte_read32((void *)((size_t)ioaddr + EQOS_DMA_MODE));

	if (dma_cfg->dche)
		value |= EQOS_DMA_BUS_MODE_DCHE;
	...
	dma_addr_t dma_tx_phy, uint32_t chan)
{
	...
	value = rte_read32((void *)((size_t) ioaddr + EQOS_DMA_CHAN_TX_CONTROL(chan)));
	value = value | (txpbl << EQOS_DMA_BUS_MODE_PBL_SHIFT);

	/* Enable OSP to get best performance */
	value |= EQOS_DMA_CONTROL_OSP;
	/* Disable the enhanced descriptor */
	value &= ~EQOS_DMA_CONTROL_EDSE;

	rte_write32(value,
		(void *)((size_t) ioaddr + EQOS_DMA_CHAN_TX_CONTROL(chan)));

	rte_write32(upper_32_bits(dma_tx_phy),
		(void *)((size_t) ioaddr + EQOS_DMA_CHAN_TX_BASE_ADDR_HI(chan)));
	rte_write32(lower_32_bits(dma_tx_phy),
		(void *)((size_t) ioaddr + EQOS_DMA_CHAN_TX_BASE_ADDR(chan)));
}

static void enetqos_set_tx_ring_len(void *ioaddr, uint32_t len, uint32_t chan)
{
	rte_write32(len, (void *)((size_t) ioaddr + EQOS_DMA_CHAN_TX_RING_LEN(chan)));
}

static void enetqos_set_rx_ring_len(void *ioaddr, uint32_t len, uint32_t chan)
{
	rte_write32(len, (void *)((size_t) ioaddr + EQOS_DMA_CHAN_RX_RING_LEN(chan)));
}
...
{
	uint32_t value = rte_read32((void *)((size_t) ioaddr + EQOS_MAC_CONFIGURATION));

	value |= EQOS_MAC_CORE_INIT;
	value |= EQOS_MAC_CONFIG_TE;
	value |= EQOS_MAC_CONFIG_RE;

	rte_write32(value, (void *)((size_t) ioaddr + EQOS_MAC_CONFIGURATION));
}

static void enetqos_dma_tx_mode(void *ioaddr, int mode,
	uint32_t channel, int fifosz, uint8_t qmode)
{
	uint32_t mtl_tx_op_mode = rte_read32((void *)((size_t)ioaddr + EQOS_MTL_CHAN_TX_OP_MODE(channel)));
	unsigned int tqs = fifosz / MTL_QUEUE_BLK - 1;

	if (mode == SF_DMA_MODE) {
		/* enable TX store and forward mode */
		mtl_tx_op_mode |= EQOS_MTL_OP_MODE_TSF;
	}

	mtl_tx_op_mode &= ~EQOS_MTL_OP_MODE_TXQEN_MASK;
	if (qmode != MTL_QUEUE_AVB_MODE)
		mtl_tx_op_mode |= EQOS_MTL_OP_MODE_TXQEN;
	else
		mtl_tx_op_mode |= EQOS_MTL_OP_MODE_TXQEN_AV;
	mtl_tx_op_mode &= ~EQOS_MTL_OP_MODE_TQS_MASK;
	mtl_tx_op_mode |= tqs << EQOS_MTL_OP_MODE_TQS_SHIFT;

	rte_write32(mtl_tx_op_mode,
		(void *)((size_t) ioaddr + EQOS_MTL_CHAN_TX_OP_MODE(channel)));
}
	...
	uint32_t channel, int fifosz, uint8_t qmode)
{
	...
	mtl_rx_op_mode = rte_read32((void *)((size_t)ioaddr + EQOS_MTL_CHAN_RX_OP_MODE(channel)));

	if (mode == SF_DMA_MODE) {
		/* enable RX store and forward mode */
		mtl_rx_op_mode |= EQOS_MTL_OP_MODE_RSF;
	}

	mtl_rx_op_mode &= ~EQOS_MTL_OP_MODE_RQS_MASK;
	mtl_rx_op_mode |= rqs << EQOS_MTL_OP_MODE_RQS_SHIFT;
	...
	if ((fifosz >= FIFO_SIZE_4KiB) && (qmode != MTL_QUEUE_AVB_MODE)) {
		...
		mtl_rx_op_mode |= EQOS_MTL_OP_MODE_EHFC;
		...
		mtl_rx_op_mode &= ~EQOS_MTL_OP_MODE_RFD_MASK;
		mtl_rx_op_mode |= rfd << EQOS_MTL_OP_MODE_RFD_SHIFT;

		mtl_rx_op_mode &= ~EQOS_MTL_OP_MODE_RFA_MASK;
		mtl_rx_op_mode |= rfa << EQOS_MTL_OP_MODE_RFA_SHIFT;
	}

	rte_write32(mtl_rx_op_mode, (void *)((size_t)ioaddr + EQOS_MTL_CHAN_RX_OP_MODE(channel)));
}
...
static void enetqos_map_mtl_dma(void *ioaddr, uint32_t queue, uint32_t chan)
{
	...
	if (queue < (MTL_MAX_RX_QUEUES / 2))
		value = rte_read32((void *)((size_t)ioaddr + EQOS_MTL_RXQ_DMA_MAP0));
	else
		value = rte_read32((void *)((size_t)ioaddr + EQOS_MTL_RXQ_DMA_MAP1));

	if (queue == 0 || queue == (MTL_MAX_RX_QUEUES / 2)) {
		value &= ~EQOS_MTL_RXQ_DMA_Q04MDMACH_MASK;
		value |= EQOS_MTL_RXQ_DMA_Q04MDMACH(chan);
	} else {
		value &= ~EQOS_MTL_RXQ_DMA_QXMDMACH_MASK(queue);
		value |= EQOS_MTL_RXQ_DMA_QXMDMACH(chan, queue);
	}

	if (queue < (MTL_MAX_RX_QUEUES / 2))
		rte_write32(value, (void *)((size_t)ioaddr + EQOS_MTL_RXQ_DMA_MAP0));
	else
		rte_write32(value, (void *)((size_t)ioaddr + EQOS_MTL_RXQ_DMA_MAP1));
}
...
{
	uint32_t value = rte_read32((void *)((size_t)ioaddr + EQOS_MAC_RXQ_CTRL0));

	value &= EQOS_MAC_RX_QUEUE_CLEAR(queue);

	if (mode == MTL_QUEUE_AVB_MODE)
		value |= EQOS_MAC_RX_AV_QUEUE_ENABLE(queue);
	else if (mode == MTL_QUEUE_DCB_MODE)
		value |= EQOS_MAC_RX_DCB_QUEUE_ENABLE(queue);

	rte_write32(value, (void *)((size_t)ioaddr + EQOS_MAC_RXQ_CTRL0));
}
...
{
	uint32_t value = rte_read32((void *)((size_t)ioaddr + EQOS_DMA_CHAN_TX_CONTROL(chan)));

	/* Start Transmission */
	value |= EQOS_DMA_CONTROL_ST;
	rte_write32(value,
		(void *)((size_t) ioaddr + EQOS_DMA_CHAN_TX_CONTROL(chan)));

	value = rte_read32((void *)((size_t) ioaddr + EQOS_MAC_CONFIGURATION));
...
{
	uint32_t value = rte_read32((void *)((size_t)ioaddr + EQOS_DMA_CHAN_RX_CONTROL(chan)));

	/* Start Receive */
	value |= EQOS_DMA_CONTROL_SR;
	rte_write32(value,
		(void *)((size_t) ioaddr + EQOS_DMA_CHAN_RX_CONTROL(chan)));
			...
			dma_addr_t dma_rx_phy, uint32_t chan)
{
	...
	value = rte_read32((void *)((size_t)ioaddr + EQOS_DMA_CHAN_RX_CONTROL(chan)));
	value = value | (rxpbl << EQOS_DMA_BUS_MODE_RPBL_SHIFT);
	rte_write32(value, (void *)((size_t)ioaddr + EQOS_DMA_CHAN_RX_CONTROL(chan)));

	rte_write32(upper_32_bits(dma_rx_phy),
		(void *)((size_t)ioaddr + EQOS_DMA_CHAN_RX_BASE_ADDR_HI(chan)));
	rte_write32(lower_32_bits(dma_rx_phy),
		(void *)((size_t)ioaddr + EQOS_DMA_CHAN_RX_BASE_ADDR(chan)));
}
...
static void enetqos_set_dma_bfsize(void *ioaddr, int bfsize, uint32_t chan)
{
	...
	value &= ~EQOS_DMA_RBSZ_MASK;
	value |= (bfsize << EQOS_DMA_RBSZ_SHIFT) & EQOS_DMA_RBSZ_MASK;
	...
	struct rte_mempool *mb_pool)
{
	...
	for (i = 0; i < nb_rx_desc; i++) {
		 ...
		 */
		p->des3 |= rte_cpu_to_le_32(EQOS_RDES3_OWN | EQOS_RDES3_BUFFER1_VALID_ADDR);
	}
	...
	struct rte_ether_addr *addr)
{
	...
	high = EQOS_MAC_ADDR_HIGH(0);
	low = EQOS_MAC_ADDR_LOW(0);
	data = (addr->addr_bytes[5] << 8) | addr->addr_bytes[4];

	data |= (ENETQOS_CHAN0 << EQOS_MAC_HI_DCS_SHIFT);
	rte_write32(data | EQOS_MAC_HI_REG_AE,
...
enetqos_promiscuous_enable(struct rte_eth_dev *dev)
{
	...
	value = rte_read32((void *)((size_t) priv->ioaddr + EQOS_MAC_PKT_FILTER));
	value |= EQOS_MAC_PKT_FILTER_PR;

	rte_write32(value,
		(void *)((size_t) priv->ioaddr + EQOS_MAC_PKT_FILTER));
...
{
	uint32_t value = rte_read32((void *)((size_t) priv->ioaddr + EQOS_MAC_CONFIGURATION));
	value &= ~(EQOS_MAC_CORE_INIT | EQOS_MAC_CONFIG_TE | EQOS_MAC_CONFIG_RE);
	rte_write32(value, (void *)((size_t) priv->ioaddr + EQOS_MAC_CONFIGURATION));
}
