		  ...
		  uint16_t bpid)
{
	...
	if (RTE_MBUF_DIRECT(mbuf)
		&& (mbuf->data_off >
		(mbuf->nb_segs * sizeof(struct qbman_sge) + offset))) {
		temp = mbuf;
...
eth_mbuf_to_fd(struct rte_mbuf *mbuf,
	       ...
	       uint32_t *free_count,
	       uint32_t pkt_id,
	       uint16_t bpid) __attribute__((unused));
...
dump_err_pkts(struct dpaa2_queue *dpaa2_q)
{
	...
	do {
		...
		DPAA2_PMD_ERR(
			"\n\n[%d] error packet on port[%d]: fd_off: %d, fd_err: %x, fas_status: %x",
