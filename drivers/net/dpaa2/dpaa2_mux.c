			  ...
			  struct rte_flow_action *actions[])
{
	...
	key_iova = (void *)((uint64_t)flow + sizeof(struct rte_flow));
	mask_iova = (void *)((uint64_t)key_iova + DIST_PARAM_IOVA_SIZE);
	...
	ret = dpdmux_add_custom_cls_entry(&dpdmux_dev->dpdmux, CMD_PRI_LOW,
		dpdmux_dev->token, &flow->rule, &dpdmux_action);
	if (ret) {
			   ...
			   int dpdmux_id)
{
	...
	ret = dpdmux_open(&dpdmux_dev->dpdmux,
			CMD_PRI_LOW, dpdmux_id, &dpdmux_dev->token);
	if (ret) {
