			    ...
			    struct rte_tm_error *error)
{
	...
	if (level_id == LNI_LEVEL) { /* Root node (LNI) */
		...
		cap->nonleaf.sched_n_children_max = priv->num_tx_tc; /* no. of class queues per channel */
		...
		cap->n_nodes_max = priv->num_tx_tc * priv->num_channels; /* queues per channels * channel */
...
dpaa2_tm_configure_queue(struct rte_eth_dev *dev, struct dpaa2_tm_node *node)
{
	...
	/*TODO: add support in TM framework,
	 * currently enabling it by default */
		...
		LIST_FOREACH(leaf_node, &priv->nodes, next) {
			...
			LIST_FOREACH(temp_leaf_node, &priv->nodes, next) {
				...
				if (leaf_node->priority == temp_leaf_node->priority) {
					...
					temp_leaf_dpaa2_q = (struct dpaa2_queue *)dev->data->tx_queues[temp_leaf_node->id];
					temp_leaf_tc_id = temp_leaf_dpaa2_q->tc_index;
					if (wfq_grp == 0) {
						prio_cfg.tc_sched[temp_leaf_tc_id].mode = DPNI_TX_SCHED_WEIGHTED_A;
						/* DPAA2 platform support weight in multiple of 100 */
						prio_cfg.tc_sched[temp_leaf_tc_id].delta_bandwidth = temp_leaf_node->weight * 100;
					} else if (wfq_grp == 1) {
						prio_cfg.tc_sched[temp_leaf_tc_id].mode = DPNI_TX_SCHED_WEIGHTED_B;
						prio_cfg.tc_sched[temp_leaf_tc_id].delta_bandwidth = temp_leaf_node->weight * 100;
					} else {
			...
			if (is_wfq_grp) {
				if (wfq_grp == 0) {
					prio_cfg.tc_sched[leaf_tc_id].mode = DPNI_TX_SCHED_WEIGHTED_A;
					prio_cfg.tc_sched[leaf_tc_id].delta_bandwidth = leaf_node->weight * 100;
					prio_cfg.prio_group_A = leaf_node->priority;
				} else if (wfq_grp == 1) {
					prio_cfg.tc_sched[leaf_tc_id].mode = DPNI_TX_SCHED_WEIGHTED_B;
					prio_cfg.tc_sched[leaf_tc_id].delta_bandwidth = leaf_node->weight * 100;
