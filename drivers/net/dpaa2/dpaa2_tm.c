		       ...
		       struct rte_tm_error *error)
{
	...
	LIST_FOREACH(node, &priv->nodes, next) {
		if (node->type == 0/*root node*/) {
			...
			tx_cr_shaper.rate_limit = node->profile->params.committed.rate / (1024 * 1024);
			...
			LIST_FOREACH(temp_node, &priv->nodes, next) {
				...
				if (node->priority == temp_node->priority) {
					if (wfq_grp == 0) {
						prio_cfg.tc_sched[temp_node->id].mode = DPNI_TX_SCHED_WEIGHTED_A;
						/* DPDK support lowest weight 1 and DPAA2 platform 100 */
						prio_cfg.tc_sched[temp_node->id].delta_bandwidth = temp_node->weight + 99;
					} else if (wfq_grp == 1) {
						prio_cfg.tc_sched[temp_node->id].mode = DPNI_TX_SCHED_WEIGHTED_B;
						prio_cfg.tc_sched[temp_node->id].delta_bandwidth = temp_node->weight + 99;
					} else {
			...
			if (is_wfq_grp) {
				if (wfq_grp == 0) {
					prio_cfg.tc_sched[node->id].mode = DPNI_TX_SCHED_WEIGHTED_A;
					prio_cfg.tc_sched[node->id].delta_bandwidth = node->weight + 99;
					prio_cfg.prio_group_A = node->priority;
				} else if (wfq_grp == 1) {
					prio_cfg.tc_sched[node->id].mode = DPNI_TX_SCHED_WEIGHTED_B;
					prio_cfg.tc_sched[node->id].delta_bandwidth = node->weight + 99;
