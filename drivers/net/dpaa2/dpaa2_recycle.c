 ...
 * According to perf benchmark,
 * the configuration priority order is:
 * 1) dpni -dpmac -single serdes lan loop back.
 * 2) dpni -dpmac TX loop back.
 * 3) dpni self.
 */
int
dpaa2_dev_recycle_config(struct rte_eth_dev *eth_dev)
{
	...
	if (dpaa2_dev->ep_dev_type == DPAA2_MAC) {
		 ...
		 * serdes loopback has higher perf according to test.
		 */
		...
		ret = dpni_set_port_cfg(dpni_dev, CMD_PRI_LOW,
				...
				&port_cfg);
		if (ret) {
			DPAA2_PMD_ERR("Error(%d) to enable TX loopback", ret);
