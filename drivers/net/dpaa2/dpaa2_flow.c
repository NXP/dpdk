			 ...
			 int *device_configured)
{
	...
	const char zero_cmp[ETHER_ADDR_LEN] = {0};
	...
	if (memcmp((const char *)&mask->src, zero_cmp, ETHER_ADDR_LEN)) {
	...
	if (memcmp((const char *)&mask->dst, zero_cmp, ETHER_ADDR_LEN)) {
		index = dpaa2_flow_extract_search(
				&priv->extract.qos_key_extract.dpkg,
			  ...
			  int *device_configured)
{
	...
	if (!spec) {
		...
		proto.eth_type = rte_cpu_to_be_16(ETHER_TYPE_VLAN);
	...
	uint32_t group)
{
	...
	if (pattern->type == RTE_FLOW_ITEM_TYPE_IPV4)
		proto.eth_type = rte_cpu_to_be_16(ETHER_TYPE_IPv4);
	else
		proto.eth_type = rte_cpu_to_be_16(ETHER_TYPE_IPv6);
	ret = dpaa2_flow_proto_discrimination_rule(priv, flow, proto, group);
	if (ret) {
			 ...
			 int *device_configured)
{
	...
	if (prev_key_size < spec->length) {
		ret = dpaa2_flow_extract_add_raw(
			&priv->extract.qos_key_extract, spec->length);
		if (ret) {
		...
		ret = dpaa2_flow_extract_add_raw(
			&priv->extract.tc_key_extract[group], spec->length);
		if (ret) {
