...
struct __rte_packed_begin pcie_ctrl_cap_head {
	uint8_t cap_id;
	uint8_t next_offset;
} __rte_packed_end;

struct __rte_packed_begin pcie_ctrl_sriov_cap {
...
struct __rte_packed_begin pcie_ctrl_ext_cap {
	uint32_t ext_cap_id:16;
	uint32_t cap_ver:4;
	uint32_t next_cap_off:12;
} __rte_packed_end;

struct __rte_packed_begin pcie_ctrl_ext_sriov_cap_ctl {
...
struct __rte_packed_begin pcie_ctrl_ext_sriov_cap {
...
struct __rte_packed_begin pcie_ctrl_cfg {
	uint16_t vendor_id;
	uint16_t device_id;
...
struct __rte_packed_begin pcie_ctrl_msix_entry {
	uint32_t msg_laddr;
	uint32_t msg_haddr;
	uint32_t msg_data;
	uint32_t vec_ctl;
...
struct __rte_packed_begin pcie_ctrl_msix_pba_entry {
	uint64_t pba_entry;
} __rte_packed_end;

#define BITS_PER_PBA_ENTRY (sizeof(struct pcie_ctrl_msix_pba_entry) * 8)

struct __rte_packed_begin pcie_ctrl_bar_size_mask {
	uint32_t rsv[4];
