...
fslmc_io_virt2phy(const void *virtaddr)
{
	...
	while (getdelim(&line, &linesz, '\n', fp) > 0) {
		...
		tmp[n] = 0;
		ret = sscanf(tmp, "%lx-%lx", &start, &end);
		if (ret != 2)
			continue;
		if (va < start || va >= end)
			continue;
...
fslmc_map_dma(uint64_t vaddr, rte_iova_t iovaddr, size_t len)
{
	...
	if (phy == RTE_BAD_IOVA) {
		phy = fslmc_io_virt2phy((const void *)vaddr);
		if (phy == RTE_BAD_IOVA)
			return -ENOMEM;
		is_io = 1;
	...
	TAILQ_FOREACH(dmaseg, &fslmc_memsegs, next) {
		if (!((vaddr + len) <= dmaseg->vaddr ||
			...
			DPAA2_BUS_ERR("MEM: New VA Range(%lx ~ %lx)",
				vaddr, vaddr + len);
			DPAA2_BUS_ERR("MEM: Overlap with (%lx ~ %lx)",
				dmaseg->vaddr,
		...
		if (!((iovaddr + len) <= dmaseg->iova ||
			...
			DPAA2_BUS_ERR("MEM: New IOVA Range(%lx ~ %lx)",
				iovaddr, iovaddr + len);
			DPAA2_BUS_ERR("MEM: Overlap with (%lx ~ %lx)",
				dmaseg->iova,
...
io_mapping_check:
	TAILQ_FOREACH(dmaseg, &fslmc_iosegs, next) {
		if (!((vaddr + len) <= dmaseg->vaddr ||
			(dmaseg->vaddr + dmaseg->size) <= vaddr)) {
			DPAA2_BUS_ERR("IO: New VA Range(%lx ~ %lx)",
				vaddr, vaddr + len);
			DPAA2_BUS_ERR("IO: Overlap with (%lx ~ %lx)",
				dmaseg->vaddr,
				dmaseg->vaddr + dmaseg->size);
			return -EEXIST;
		}
		if (!((iovaddr + len) <= dmaseg->iova ||
			(dmaseg->iova + dmaseg->size) <= iovaddr)) {
			DPAA2_BUS_ERR("IO: New IOVA Range(%lx ~ %lx)",
				iovaddr, iovaddr + len);
			DPAA2_BUS_ERR("IO: Overlap with (%lx ~ %lx)",
				dmaseg->iova,
				dmaseg->iova + dmaseg->size);
...
fslmc_unmap_dma(uint64_t vaddr, uint64_t iovaddr, size_t len)
{
	...
	if (!dmaseg) {
		DPAA2_BUS_ERR("IOVA(%lx) with length(%lx) not mapped",
			iovaddr, len);
		return 0;
	}
	...
	ret = ioctl(fslmc_vfio_container_fd(), VFIO_IOMMU_UNMAP_DMA,
		&dma_unmap);
	if (ret) {
		DPAA2_BUS_ERR("DMA un-map IOVA(%lx~%lx) err(%d)",
	...
	uint64_t size)
{
	...
	TAILQ_FOREACH(dmaseg, &fslmc_memsegs, next) {
		if (iova >= dmaseg->iova &&
			...
			return (void *)(dmaseg->vaddr + iova - dmaseg->iova);
	}
...
{
	if (likely(fslmc_mem_va2iova != RTE_BAD_IOVA))
		return (void *)(iova - fslmc_mem_va2iova);
	...
	size_t len, void *arg __rte_unused)
{
	...
	while (cur_len < len) {
		...
		DPAA2_BUS_DEBUG("%s, va=%p, virt=%lx, iova=%lx, len=%zu",
...
static void *fslmc_vfio_map_mcp_obj(const char *mcp_obj)
{
	void *v_addr = MAP_FAILED;
	int32_t ret, mc_fd;
	...
	v_addr = mmap(NULL, reg_info.size,
...
fslmc_vfio_process_mcp(struct rte_dpaa2_device *dev)
{
	...
	v_addr = fslmc_vfio_map_mcp_obj(dev->device.name);
