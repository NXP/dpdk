...
dpaa_sec_prep_pdcp_cdb(dpaa_sec_session *ses)
{
	...
	if (ses->pdcp.sdap_enabled) {
		...
		if (nb_keys_to_inline >= 1) {
			cipherdata.key = (size_t)rte_dpaa_mem_vtop((void *)(size_t)cipherdata.key);
			cipherdata.key_type = RTA_DATA_PTR;
		}
		if (nb_keys_to_inline >= 2) {
			authdata.key = (size_t)rte_dpaa_mem_vtop((void *)(size_t)authdata.key);
...
dpaa_sec_deq(struct dpaa_sec_qp *qp, struct rte_crypto_op **ops, int nb_ops)
{
	...
	fq = &qp->outq;
	/*
	 * There are 2 modes in volatile dequeue: EXACT or non-EXACT.
	 * In non-EXACT mode, hardware can provide up-to ~20% increase
	 * in performance numbers. However it can result in provide up-to
	 * 2 extra buffers for a given request.
	 * we are keeping Default option as EXACT mode, but if we want to
	 * use non-EXACT mode then we can use below code.
	 *
