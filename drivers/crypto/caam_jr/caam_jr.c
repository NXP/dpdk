...
#else
	memset(&ctx->sg, 0, MAX_SG_ENTRIES*16);
#endif
...
/* Start queue pair */
static int
caam_jr_queue_pair_start(__rte_unused struct rte_cryptodev *dev,
...
/* Stop queue pair */
static int
caam_jr_queue_pair_stop(__rte_unused struct rte_cryptodev *dev,
			 __rte_unused uint16_t queue_pair_id)
{
	PMD_INIT_FUNC_TRACE();
...
static unsigned int
caam_jr_session_get_size(struct rte_cryptodev *dev __rte_unused)
{
	PMD_INIT_FUNC_TRACE();
...
static int
caam_jr_qp_attach_sess(struct rte_cryptodev *dev, uint16_t qp_id, void *ses)
{
	struct caam_jr_session *sess = ses;
	...
	sess->qp = qp;
	ret = caam_jr_prep_cdb(sess);
	if (ret)
		CAAM_JR_ERR("error while preparing cdb\n");

	return ret;
}

static int
caam_jr_qp_detach_sess(struct rte_cryptodev *dev __rte_unused,
		uint16_t qp_id __rte_unused, void *ses)
{
	struct caam_jr_session *sess = ses;
	int ret = 0;
...
static int
caam_jr_session_configure(struct rte_cryptodev *dev,
		struct rte_crypto_sym_xform *xform,
...
static void
caam_jr_session_clear(struct rte_cryptodev *dev,
		struct rte_cryptodev_sym_session *sess)
{
...
static struct rte_cryptodev_ops caam_jr_ops = {
	...
	.queue_pair_start     = caam_jr_queue_pair_start,
	.queue_pair_stop      = caam_jr_queue_pair_stop,
	.queue_pair_count     = caam_jr_queue_pair_count,
	.session_get_size     = caam_jr_session_get_size,
	.session_configure    = caam_jr_session_configure,
	.session_clear        = caam_jr_session_clear,
	.qp_attach_session    = caam_jr_qp_attach_sess,
	.qp_detach_session    = caam_jr_qp_detach_sess,
};
...
init_job_ring(void *reg_base_addr, uint32_t irq_id)
{
	...
	for (int i = 0; i < MAX_SEC_JOB_RINGS; i++) {
...
RTE_PMD_REGISTER_PARAM_STRING(CRYPTODEV_NAME_CAAM_JR_PMD,
	...
	"max_nb_sessions=<int>"
	"socket_id=<int>");
RTE_PMD_REGISTER_CRYPTO_DRIVER(caam_jr_crypto_drv, cryptodev_caam_jr_drv,
		cryptodev_driver_id);

RTE_INIT(caam_jr_init_log);
static void
caam_jr_init_log(void)
{
