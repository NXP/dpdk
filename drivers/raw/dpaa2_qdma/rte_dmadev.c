...
static int dmadev_logtype;

RTE_INIT(dmadev_init_log)
{
	dmadev_logtype = rte_log_register("dmadev");
	if (dmadev_logtype >= 0)
		rte_log_set_level(dmadev_logtype, RTE_LOG_INFO);
}

#define RTE_DMA_LOG(level, fmt, args...) \
	rte_log(RTE_LOG_ ## level, dmadev_logtype, "dma: " \
		fmt "\n", ##args)
