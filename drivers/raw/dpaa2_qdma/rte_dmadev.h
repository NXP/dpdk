...
#ifdef __rte_experimental
#undef __rte_experimental
 ...
 * so user must ensure that all memory addresses are valid and
 * accessible by HW.
 */
 ...
 *
 * Enum with the options for the channel status, either idle,
 * active or halted due to error.
 ...
 * This function is primarily designed for testing use,
 * as it allows a process to check if all jobs are completed,
 * without actually gathering completions from those jobs.
 *
 * @param dev_id
 *   The identifier of the device.
 * @param vchan
...
__rte_experimental
int
rte_dma_vchan_status(int16_t dev_id, uint16_t vchan,
