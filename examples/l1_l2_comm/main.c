...
static uint32_t s_tb_size = 17500;

static uint32_t s_sdu_size = 1500;

static uint32_t s_port_id = RTE_MAX_ETHPORTS;
...
static enum up_down_handle
s_l1_l2_handle = (UP_LINK_ENABLE | DOWN_LINK_ENABLE);
...
#define CORE_RUNNING 1
static uint32_t s_core_num;
static uint32_t s_core_queue[RTE_MAX_LCORE];
static uint32_t s_core_running_idx[RTE_MAX_LCORE];

#define L1_L2_MAX_CHAIN_NB (s_tb_size / s_sdu_size + 1)
...
{
	int i;
	uint32_t remain_len = s_tb_size, nb_segs;
	uint64_t *ptick;
	uint64_t tick = rte_get_timer_cycles();

	if (remain_len % s_sdu_size)
		nb_segs = remain_len / s_sdu_size + 1;
	else
		nb_segs = remain_len / s_sdu_size;
...
l2_app_uplink_data_ready(struct rte_mbuf *mbufs)
{
	...
	return rte_eth_tx_burst(s_port_id, s_core_queue[core], &mbufs, 1);
}
...
l2_app_downlink_prepare(void)
{
	...
	if ((uint32_t)chain_count < L1_L2_MAX_CHAIN_NB) {
	...
	if (unlikely(ret != 1)) {
		...
		s_data_loop_conf.tx_statistic[core].packets += tx_pkts;
		s_data_loop_conf.tx_statistic[core].bytes += tx_bytes;
	}
}
...
recv_again:
	ret = rte_eth_rx_burst(s_port_id, s_core_queue[core], mbufs, 1);
	if (ret)
		return ret;
...
l2_app_uplink_data_process(struct rte_mbuf *mbuf)
{
	...
	avg_latency = &s_data_loop_conf.avg_latency[core];
	cyc_diff_total = &s_data_loop_conf.cyc_diff_total[core];
	packets = &s_data_loop_conf.rx_statistic[core].packets;
	while (1) {
		if (s_l2_uplink_print) {
		...
		tick_send = *ptick;
		s_data_loop_conf.cyc_diff_total[core] +=
	...
	s_data_loop_conf.rx_statistic[core].packets += nb_segs;
	s_data_loop_conf.rx_statistic[core].bytes += total_len;

	*avg_latency = (*cyc_diff_total) / (*packets) / s_l1_l2_cycs_per_us;
}
...
l1_app_downlink_data_process(struct rte_mbuf *mbuf)
{
	...
	s_data_loop_conf.rx_statistic[core].packets++;
	s_data_loop_conf.rx_statistic[core].bytes += pkt_len;
...
{
	mbuf->pkt_len = s_tb_size;
	mbuf->data_len = s_tb_size;
	mbuf->data_off = RTE_PKTMBUF_HEADROOM;
	mbuf->tso_segsz = s_sdu_size;
	/**Process data*/
...
l1_app_uplink_process(void)
{
	...
	if (mbuf->buf_len < (s_tb_size + RTE_PKTMBUF_HEADROOM)) {
		fprintf(stderr,
			"Buf(len = %d) < TB(len = %d) + header room(%d)\r\n",
			mbuf->buf_len, s_tb_size, RTE_PKTMBUF_HEADROOM);
		rte_pktmbuf_free(mbuf);
		return;
	}
...
main_loop(__attribute__((unused)) void *dummy)
{
	...
	if (s_calculate_latency && s_layer == 1) {
		sprintf(nm, "l1_mbuf_ring_%d", rte_lcore_id());
...
static int
parse_tb_size(const char *tb_size)
{
	s_tb_size = atoi(tb_size);

	return 0;
}

static int
parse_sdu_size(const char *sdu_size)
{
	s_sdu_size = atoi(sdu_size);

	return 0;
}

static int
parse_measure_latency(const char *latency)
{
	s_calculate_latency = atoi(latency);
...
#define CMD_LINE_OPT_TB_SIZE "tb-size"
#define CMD_LINE_OPT_SDU_SIZE "sdu-size"
#define CMD_LINE_OPT_LATENCY "measure-latency"
...
enum {
	...
	CMD_LINE_OPT_TB_SIZE_NUM,
	CMD_LINE_OPT_SDU_SIZE_NUM,
...
static const struct option lgopts[] = {
	...
	{CMD_LINE_OPT_TB_SIZE, 1, 0, CMD_LINE_OPT_TB_SIZE_NUM},
	{CMD_LINE_OPT_SDU_SIZE, 1, 0, CMD_LINE_OPT_SDU_SIZE_NUM},
...
parse_args(int argc, char **argv)
{
	...
	while ((opt = getopt_long(argc, argvopt, sopts,
		...
		case CMD_LINE_OPT_TB_SIZE_NUM:
			ret = parse_tb_size(optarg);
			if (ret) {
				fprintf(stderr,
					"Invalid TB size\n");
				return ret;
		...
		case CMD_LINE_OPT_SDU_SIZE_NUM:
			ret = parse_sdu_size(optarg);
			if (ret) {
				fprintf(stderr,
					"Invalid SDU size\n");
				return ret;
		...
		case CMD_LINE_OPT_LATENCY_NUM:
			ret = parse_measure_latency(optarg);
			if (ret) {
				fprintf(stderr,
					"Invalid latency test\n");
				return ret;
			}
			break;
...
loop:
	...
	for (i = 0; i < RTE_MAX_LCORE; i++) {
		rxs = &s_data_loop_conf.rx_statistic[i];
		txs = &s_data_loop_conf.tx_statistic[i];
		rx_pkts += rxs->packets;
		tx_pkts += txs->packets;
		rx_bytes += rxs->bytes;
		tx_bytes += txs->bytes;
		avg_latency += s_data_loop_conf.avg_latency[i];
	}
...
main(int argc, char **argv)
{
	...
	int ret;
	uint16_t portid, core, core_num = 0, qid;
	uint32_t nb_buf;
	...
	for (core = 0; core < RTE_MAX_LCORE; core++) {
		if (rte_lcore_is_enabled(core)) {
			s_core_queue[core] = CORE_RUNNING;
			s_core_running_idx[core_num] = core;
			core_num++;
		}
	}
	s_core_num = core_num;
	...
	if (s_layer == 1) {
		nb_buf = (nb_txd + nb_rxd) * 2 * 10;
		data_room_size = (s_tb_size + RTE_PKTMBUF_HEADROOM +
			MEMPOOL_CACHE_SIZE) /
			MEMPOOL_CACHE_SIZE * MEMPOOL_CACHE_SIZE;
	} else {
		nb_buf = (nb_txd + nb_rxd) * 2 * 32 * 10;
		data_room_size = (s_sdu_size + RTE_PKTMBUF_HEADROOM);
	}
	...
	RTE_ETH_FOREACH_DEV(portid) {
		...
		ret = rte_eth_dev_configure(portid, core_num, core_num,
			&local_port_conf);
		if (ret < 0) {
			rte_exit(EXIT_FAILURE,
		...
		for (qid = 0; qid < core_num; qid++) {
			ret = rte_eth_rx_queue_setup(portid, qid, nb_rxd, 0, &rxq_conf,
					l1_l2_mpool);
			if (ret < 0) {
				rte_exit(EXIT_FAILURE,
			...
			ret = rte_eth_tx_queue_setup(portid, qid, nb_txd, 0, txconf);
			if (ret < 0) {
				rte_exit(EXIT_FAILURE,
					"port%d txq%d setup failed(%d)\n",
					portid, qid, ret);
			}
			s_core_queue[s_core_running_idx[qid]] = qid;
		}
