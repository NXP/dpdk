...
single_icmp_reply(struct rte_mbuf *pkt)
{
	...
	rte_memcpy(eth->d_addr.addr_bytes, eth->s_addr.addr_bytes,
		RTE_ETHER_ADDR_LEN);
	rte_memcpy(eth->s_addr.addr_bytes, s_tap_ethaddr,
		RTE_ETHER_ADDR_LEN);
	iph4->hdr_checksum = 0;
...
ipsec_ike_cryptodev_init(void)
{
	...
	qp_conf.mp_session_private = s_mem_ctx.session_priv_pool;
