...
#define ENABLE_ETHERNET_ONLY 0

void eth_rx_cb(void *outbuf, void *cntx);
void crypto_cb(void *outbuf, void *cntx);
...
void eth_rx_cb(void *outbuf, void *cntx __rte_unused)
{
	struct rte_mbuf *mbuf = outbuf;
	struct pdcp_mbuf_metadata *priv;
	struct rte_crypto_sym_op *sym_cop;
	struct rte_crypto_op *op_buffer[1];
	int ret;
	unsigned int cdev_id = rte_lcore_id();
#if ENABLE_ETHERNET_ONLY
	...
	rte_security_attach_session(&priv->cop, port_cparams[cdev_id].pdcp_sess);
	op_buffer[0] = &priv->cop;
	ret = rte_cryptodev_enqueue_burst(cdev_id, 0/*queue_id*/, op_buffer, 1);
	if (ret < 1) {
		RTE_LOG_DP(DEBUG, L2FWD, "Cryptodev %u queue %u:"
...
void crypto_cb(void *outbuf, void *cntx __rte_unused)
{
	struct rte_crypto_op *cop = outbuf;
...
l2fwd_main_loop(struct l2fwd_crypto_options *options)
{
	...
	uint16_t portid = 0, ev_portid = lcore_id;
	struct lcore_queue_conf *qconf = &lcore_queue_conf[lcore_id];
	struct rte_security_session *pdcp_sess;
	int retval, eventfd, nfds;
	fd_set readset;
	...
	retval = rte_cryptodev_socket_id(port_cparams[i].dev_id);
	sess_mp = session_pool_socket[(uint8_t)retval];
	...
	eventfd = rte_dpaa_event_get_epoll_fd(ev_portid);
	if (eventfd <= 0)
		rte_exit(EXIT_FAILURE, "error in getting eventfd[%d]\n", ev_portid);
	...
	while (1) {
		 ...
		 */
		nb_rx = rte_dpaa_event_process_packets(adapter_id, ev_portid, 1);
		if (nb_rx <= 0) {
			nfds = eventfd + 1;
			FD_ZERO(&readset);
			FD_SET(eventfd, &readset);

			retval = select(nfds, &readset, NULL, NULL, NULL);
			if (retval)
				rte_dpaa_event_clear_irq(eventfd, &readset);
...
eventdev_eth_configure(struct l2fwd_crypto_options *options)
{
	...
	for (i = 0; i < options->nb_port_params; i++) {
		for (j = 0; j < options->port_params[i].num_queues; j++) {
			...
			struct rte_dpaa_dev_qconf_update_t cb_update_conf;

			cb_update_conf.process_packet_cb = eth_rx_cb;
			cb_update_conf.cntx = NULL;
			ret = rte_dpaa_event_eth_rx_queue_update(adapter_id,
					options->port_params[i].port_id,
					queue_id,
					&cb_update_conf);
			if (ret < 0)
				rte_exit(EXIT_FAILURE,
					"rte_dpaa_event_eth_rx_queue_update: err=%d\n",
					ret);
		}
	}
...
eventdev_crypto_configure(struct l2fwd_crypto_options *options)
{
	...
	for (i = 0; i < options->nb_port_params; i++) {
		...
		cb_update_conf.process_packet_cb = crypto_cb;
		cb_update_conf.cntx = NULL;
		ret = rte_dpaa_event_crypto_qp_update(adapter_id,
					ev.flow_id,
					0/*queue_id*/,
					&cb_update_conf);
		if (ret < 0)
