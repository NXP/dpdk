...
eventdev_crypto_configure(struct l2fwd_crypto_options *options)
{
	...
	uint8_t queue_id = 0, queue_prio = 0;
	int ret, i;
	struct rte_event_crypto_adapter_queue_conf crypto_ev = {0};
	...
	for (i = 0; i < options->nb_port_params; i++) {
		crypto_ev.ev.flow_id = lcore_queue_conf[i].cryptodev_list[0];
		crypto_ev.ev.sched_type = options->sched_type;
		crypto_ev.ev.event_type = RTE_EVENT_TYPE_CRYPTODEV;
		crypto_ev.ev.priority = queue_prio;
		crypto_ev.ev.queue_id = queue_id;

		ret = rte_event_crypto_adapter_queue_pair_add(adapter_id,
				crypto_ev.ev.flow_id, 0/*cdev_qp->queue_id*/,
				&crypto_ev);
		if (ret < 0)
			rte_exit(EXIT_FAILURE,
