...
ecpri_port_flow_configure(uint16_t portid, uint8_t nb_rx_queue)
{
	...
	eth_hdr = (struct ether_hdr *)spec_pattern;
	eth_hdr->ether_type = rte_cpu_to_be_16(ETHERTYPE_ECPRI);
	spec.offset = 0;
	spec.pattern = spec_pattern;
	spec.length = sizeof(struct ether_hdr) + sizeof(ecpri_header_t) +
		sizeof(ecpri_iq_data_t);

	/* Set mask (pattern) */
	mask_pattern = rte_zmalloc(NULL, 128, 0);
	eth_hdr = (struct ether_hdr *)mask_pattern;
	eth_hdr->ether_type = 0xFFFF;
	iq = (ecpri_iq_data_t *)(mask_pattern + sizeof(struct ether_hdr) +
	...
	mask.length = sizeof(struct ether_hdr) + sizeof(ecpri_header_t) +
		sizeof(ecpri_iq_data_t);
